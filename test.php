...
<body>
    ...
    if (isset($_POST["supprime"])) {
        $dateASupprimer = $_POST["dateASupprimer"];
        supprimerConversions($historique, $dateASupprimer);
    }
    ...
    function supprimerConversions($fichier, $dateASupprimer) {
        $lignes = file($fichier);
        $nouvelHistorique = array();

        foreach ($lignes as $ligne) {
            if (strpos($ligne, $dateASupprimer) === false) {
                $nouvelHistorique[] = $ligne;
            }
        }

        file_put_contents($fichier, implode('', $nouvelHistorique));
    }
    ?>
    ...
    if ($lines !== false) {
        ...
        foreach ($historiqueParDate as $date => $conversions) {
            ...
            echo "<input type='hidden' name='dateASupprimer' value='$date'>";
            echo "<button name='supprime' type='submit'>Supprimer</button>";
            echo "</form>";
            echo "<br><br>";
        }
